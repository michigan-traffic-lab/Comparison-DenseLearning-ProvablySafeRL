from gym_commonroad_sumo.NDE_RL_NADE.mtlsp.vehicle.vehicle import VehicleList, Vehicle, VehicleDynamics
...
class BaseEnv(object):
        ...
        for veh_id in veh_id_list:
            if "CAV" not in veh_id:
                ...
                if not self.simulator.replay_flag or self.simulator.replay_CAV_flag:
                    ...
                    veh = Vehicle(id=veh_id, controller=Controller(), simulator=self.simulator)
            if veh.type in self.independent_controller_dict and self.independent_controller_dict[veh.type] is not None:
    ...
    def terminate_check(self):
        ...
        if self.simulator.replay_flag and self.simulator.input is None:
            time_step = self.simulator.sumo_time_stamp
            sim_length = max(list(self.simulator.input_trajs.keys()))
            if time_step > float(sim_length):
                stop = True
        if stop:
