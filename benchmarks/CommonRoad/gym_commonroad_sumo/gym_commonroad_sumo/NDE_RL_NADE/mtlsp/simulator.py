...
class Simulator(object):    
    ...
    def __init__(self,
        ...
        self.replay_fcdjson = replay_fcdjson
        self.replay_single = replay_single
        ...
        if replay_flag:
            if input_path is not None:
                ...
                trajs_exist_flag = os.path.isdir(single_input_path)
            else:
                trajs_exist_flag = False
            if trajs_exist_flag:
                if replay_fcdjson:
                    input_file_list = glob.glob(input_path+"/crash/*.fcd.json")
                elif replay_single:
                    input_file_list = glob.glob(single_input_path+"/*.fcd.json")
                else:
                    input_file_list = glob.glob(input_path+"/crash/*.fcd.xml")
                for file in input_file_list:
                    file_name = file.split("/")[-1]
                    if replay_fcdjson or replay_single:
                        episode_num = int(file_name.replace(".fcd.json",""))
                        json_file = file.replace("fcd.json","json")
                        if replay_single:
                    ...
                    if replay_single:
                        self.input_trajs = utils.load_trajs_from_jsonfile(self.input_file_dict[self.episode][0])
            elif self.input is not None:
                self.input_file_dict[self.input[2][0]] = []
            else:
    ...
    def start(self):
        ...
        if self.sumo_control_state:
            sumoCmd = [self.sumo_binary, "-c", self.sumo_config_file_path, "--step-length", str(self.step_size), "--random", "--collision.mingap-factor", "0", "--collision.action", "warn", "--no-warnings"]
            sumoCmd += ["--precision", "6"]
    ...
    def step(self, drl_action=None):
        ...
        if conf.record_video:
            image_name = str(int(conf.frame_index)).zfill(4)+".jpg"
            image_path = "./images/"+image_name
            traci.gui.screenshot("View #0", image_path)
            conf.set_frame_index(conf.frame_index+1)
    ...
    def soft_run(self):
        ...
        if self.replay_flag:
            ...
            if self.replay_fcdjson and self.input is None:
                self.input_trajs = utils.load_trajs_from_jsonfile(self.input_file_dict[self.episode][0])
            elif self.replay_fcdjson and self.input is not None:
                self.input_trajs = self.input[1]
                self.input_trajs_add = self.input[0]
            else:
            ...
            try:
                with open(self.input_file_dict[self.episode][1]) as f:
                    self.input_trajs_add = json.load(f)
            except:
                pass
        self.env.initialize()
        self.sumo_time_stamp = self.get_time()
    ...
    def stop(self):
        ...
        if self.started:
            ...
            if conf.record_video:
                ...
                for i in range(10):
                    shutil.copy(
                        "./images/input/black.jpg", 
                        "./images/"+str(int(conf.frame_index)).zfill(4)+".jpg"
                    )
                    conf.set_frame_index(conf.frame_index+1)
    ...
    def subscribe_vehicle_ego(vehID):
        ...
        Args:
            vehID (str): Vehicle ID.
        """
        traci.vehicle.subscribe(vehID, [tc.VAR_LENGTH, tc.VAR_POSITION, tc.VAR_SPEED, tc.VAR_LANE_INDEX, tc.VAR_ANGLE, tc.VAR_POSITION3D, tc.VAR_EDGES, tc.VAR_LANEPOSITION, tc.VAR_LANEPOSITION_LAT, tc.VAR_SPEED_LAT, tc.VAR_ROAD_ID, tc.VAR_ACCELERATION, tc.VAR_SIGNALS])
