...
from gym_commonroad_sumo.NDE_RL_NADE.controller.treesearchnadecontroller import TreeSearchNADEBackgroundController
import numpy as np
...
class NADEBVGlobalController(NDDBVGlobalController):
    ...
    def step(self, drl_action=None):
        ...
        for bv_id in self.controllable_veh_id_list:
            ...
            if self.env.simulator.replay_flag:
                bv.update()
            bv.controller.step()
        ...
        if 1:
            ...
            for bv_id in self.controllable_veh_id_list:
                ...
                if self.apply_control_permission():
                    ...
                    if not self.env.simulator.replay_flag:
                        bv.update()
    ...
    def get_reachability_range(current_pos, current_velocity, predict_time_gap=1, max_a=2, min_a=-4):
        ...
        Args:
            ...
            max_a (float): max acceleraton
            min_a (float): min acceleration

        Returns:
            reach_range (list): the upper bound and lower bound of the vehicle
        """
        lower_bound_list = []
        upper_bound_list = []
    ...
    def intersection_check(cav_reach_range_lb_list, cav_reach_range_ub_list, bv_reach_range_lb_list, bv_reach_range_ub_list):
        """return whether the two up and lb bound has intersections

        Args:
            cav_reach_range_lb_list ([type]): [description]
            cav_reach_range_ub_list ([type]): [description]
            bv_reach_range_lb_list ([type]): [description]
            bv_reach_range_ub_list ([type]): [description]

        Returns:
            [bool]: True: has intersection, False: no intersection
        """
    ...
    def get_bv_candidates(self):
        """Find the bv candidates around the av for NADE safety test.
    ...
    def collect_discriminator_input_simplified(self, full_obs, controlled_bvs_list, bv_criticality_list):
        ...
        # for vehicle in controlled_bvs_list:
            ...
            # vehicle_local_position = list(vehicle_single_obs["position"])
            # vehicle_relative_position = [vehicle_local_position[0]-CAV_global_position[0], vehicle_local_position[1]-CAV_global_position[1]]
            # vehicle_speed = vehicle_single_obs["velocity"]
            # vehicle_info_list.extend(vehicle_relative_position +[vehicle_speed])
        if len(vehicle_info_list) < total_bv_info_length:
            vehicle_info_list.extend([-1]*(total_bv_info_length - len(vehicle_info_list)))
        # bv_criticality_list = self.change_criticality_result(bv_criticality_list, controlled_bv_num)
    ...
    def collect_discriminator_input_more_bvs(self, full_obs, controlled_bvs_list, bv_criticality_list): # 19 dimension observation
        ...
        if len(vehicle_info_list) < total_bv_info_length:
            # vehicle_info_list.extend([-20, -8, -10, -20]*((total_bv_info_length - len(vehicle_info_list))/4))
    ...
    def _get_original_NADE_decision(self, controlled_bvs_list, CAV_obs, full_obs):
        ...
        # print("Update original_NADE_decision!")
        # if sum(bv_criticality_list) > 0:
        #     print(self.env.simulator.get_time(), sum(bv_criticality_list))
