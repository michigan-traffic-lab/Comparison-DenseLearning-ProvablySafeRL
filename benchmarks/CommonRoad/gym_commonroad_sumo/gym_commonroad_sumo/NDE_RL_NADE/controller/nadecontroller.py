...
class NADEBackgroundController(NDDController):
    ...
    def _BV_accelerate_challenge(v, r, rr):
        ...
        # index = np.where((conf.BV_CF_state_value == [round_r, round_rr, round_speed]).all(1))
        # assert(len(index) <= 1)
        # index = index[0]
    ...
    # # @profile
    # def Decompose_decision_old(self, CAV, SM_LC_prob, full_obs=None):
    ...
    #     """
    #     self.bv_criticality_array = np.zeros(len(conf.ACTIONS), dtype=float)
    #     bv_criticality, bv_action_idx, weight, ndd_possi, critical_possi, weight_list = -np.inf, None, None, None, None, None
    ...
    #         # if not ((0.99999 <= bv_left_prob <= 1) or (0.99999 <= bv_right_prob <= 1) or utils.is_lane_change(bv_obs["Ego"])):
    #         bv_criticality, criticality_array, bv_challenge_array = self._calculate_criticality(bv_obs, CAV, SM_LC_prob, full_obs)
    #         self.bv_challenge_array = bv_challenge_array
    #         if bv_criticality > conf.criticality_threshold:
    #             bv_action_idx, weight, ndd_possi, critical_possi, weight_list = self._sample_critical_action(criticality_array, bv_criticality, bv_pdf, bv_challenge_array)
    #         if weight is not None:
    #             weight, ndd_possi, critical_possi = weight.item(), ndd_possi.item(), critical_possi.item()
    #     return bv_criticality, bv_action_idx, weight, ndd_possi, critical_possi, weight_list
