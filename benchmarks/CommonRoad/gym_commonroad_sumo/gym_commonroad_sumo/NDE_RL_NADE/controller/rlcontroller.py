...
from gym_commonroad_sumo.NDE_RL_NADE.utils import acceleration
...
from gym_commonroad_sumo.NDE_RL_NADE.envs.nde import NDE
from gym_commonroad_sumo.NDE_RL_NADE.mtlsp.utils import update_vehicle_real_states
...
class RLController(BaseController):
    ...
    def step(self):
        ...
        if not conf.experiment_config["train_flag"]:
            action_indicator = self.get_action_indicator(
                self.vehicle.observation,
                ndd_flag=False,
                safety_flag=True,
                CAV_flag=True,
            )
            action_indicator_after_lane_conflict = self.lane_conflict_safety_check(
                self.vehicle.observation, action_indicator
            )
        else:
            action_indicator_after_lane_conflict = self.get_action_indicator(
                self.vehicle.observation,
                ndd_flag=False,
                safety_flag=False,
                CAV_flag=True,
        ...
        s, action_index = self.agent.decision(
            self.vehicle.observation,
            action_indicator_after_lane_conflict,
    ...
    def get_action_indicator(
        self, observation, ndd_flag=False, safety_flag=True, CAV_flag=False
    ):
        ...
        if CAV_flag:
            ...
            if safety_flag:
                ...
                lateral_result = BaseController._check_lateral_safety(
                    observation.information, lateral_action_indicator, CAV_flag=True
                )
                longi_result = BaseController._check_longitudinal_safety(
                    observation.information,
                    np.ones(action_shape - 2),
                    lateral_result=lateral_result,
                    CAV_flag=True,
                )
                safety_action_indicator[0], safety_action_indicator[1] = (
                    lateral_result[0],
                    lateral_result[2],
                )
    ...
    def lane_conflict_safety_check(self, observation, action_indicator_before):
        ...
        CAV_v, CAV_x, CAV_current_lane_id = (
            CAV_info["velocity"],
            CAV_info["position"][0],
            CAV_info["lane_index"],
        )
        ...
        if (CAV_current_lane_id == 0 and not action_indicator_before[0]) or (
            CAV_current_lane_id == 2 and not action_indicator_before[1]
        ):
            return action_indicator_before

        if CAV_current_lane_id == 0:
            candidate_BV_lane, CAV_ban_lane_change_id = 2, 0
        elif CAV_current_lane_id == 2:
            candidate_BV_lane, CAV_ban_lane_change_id = 0, 1
        # candidate_BV
        ...
        for veh_id in candidate_BV_dict.keys():
            ...
            if BV_x >= CAV_x:
                ...
                BV_dis = utils.cal_dis_with_start_end_speed(
                    BV_v,
                    np.clip(BV_v + acc_BV, self.v_low, self.v_high),
                    acc_BV,
                    time_interval=conf.simulation_resolution,
                    v_low=self.v_low,
                    v_high=self.v_high,
                )
                CAV_dis = utils.cal_dis_with_start_end_speed(
                    CAV_v,
                    np.clip(CAV_v + acc_CAV, self.v_low, self.v_high),
                    acc_CAV,
                    time_interval=conf.simulation_resolution,
                    v_low=self.v_low,
                    v_high=self.v_high,
...
class RLControllerNew(BaseController):
    ...
    def __init__(
        self,
        subscription_method=Simulator.subscribe_vehicle_all_information,
        controllertype="RLControllerNew",
    ):
        super().__init__(
            subscription_method=subscription_method, controllertype=controllertype
        )
        ...
        if conf.simulation_config["neuralmetric_flag"]:
            self.NN_metric = NN_Metric(
                conf.simulation_config["neuralmetric_config_path"]
            )
        else:
            self.NN_metric = None
    ...
    def install(self):
        ...
        if self.vehicle.simulator.input is not None:
            ...
            self.vehicle.lateral_speed = (
                self.vehicle.simulator.env.cav_initial_lateral_speed
            )
            self.RSS_model.update_states(
                self.vehicle.simulator.env.cav_initial_RSS_states,
                self.vehicle.simulator.env.init_time_step,
            )
    ...
    def step(self):
        ...
        if self.vehicle.simulator.env.cav_action is not None:
            ...
            if conf.RSS_flag:
                new_states = update_vehicle_real_states(
                    self.vehicle.states,
                    {
                ...
                new_lateral_v = (
                    dy / self.vehicle.step_size * 2 - self.vehicle.lateral_speed
                )
                acc_lat = (
                    new_lateral_v - self.vehicle.lateral_speed
                ) / self.vehicle.step_size
                RSS_restriction = self.RSS_model.RSS_act_CAV(
                    self.vehicle.simulator.env, {"acc_x": acc, "acc_y": acc_lat}
                )
                RSS_control = self.RSS_model.RSS_step_CAV(
                    env=self.vehicle.simulator.env
                )
                ...
                if not self.RSS_model.cav_veh.RSS_control:
                    ...
                    action_list = fsolve(
                        self.helper_solve_for_control_inputs,
                        [float(action[0]), float(action[1])],
                        args=(RSS_action,),
                    )
                    self.action = {
                        "acceleration": np.clip(
                            float(action_list[0]), self.acc_low, self.acc_high
                        ),
                        "steering_angle": np.clip(
                            float(action_list[1]), -self.angle_max, self.angle_max
                        ),
                        "additional_info": [
                            list(obs.astype("float64")),
                            RSS_restriction["CAV"],
                            RSS_control,
                        ],
                    }
            ...
            if conf.cav_agent is None:
                if (
                    not conf.load_pytorch_model
                    and conf.simulation_config["ray_ckpt_path_list"] != []
                ):
                    conf.cav_agent = conf.load_ray_agent(
                        conf.simulation_config["ray_ckpt_path_list"],
                        conf.simulation_config["slightly_critical_ray_ckpt_path_list"],
                    )
                elif (
                    conf.load_pytorch_model
                    and conf.simulation_config["pytorch_model_path_list"] != []
                ):
                    conf.cav_agent = conf.load_ray_agent(
            ...
            if len(conf.cav_agent.agent_list) == 2 and self.highly_critical:
                ...
                self.vehicle.simulator.set_vehicle_color(
                    self.vehicle.id, self.vehicle.color_purple
                )
            ...
            if not conf.d2rl_flag:
                action = conf.cav_agent.compute_action(
                    obs, self.highly_critical, self.slightly_critical
                )
            else:
            ...
            new_states = update_vehicle_real_states(
                self.vehicle.states,
                {"acceleration": float(action[0]), "steering_angle": float(action[1])},
                self.vehicle.params,
                self.vehicle.step_size,
            )
            dx = new_states[1] - self.vehicle.states[1]
            ...
            if conf.RSS_flag:
                ...
                for pair in self.RSS_model.RSS_state_dict:
                    assert pair[0] == "CAV"
                    new_RSS_state_dict[pair[1]] = dict(
                        self.RSS_model.RSS_state_dict[pair]
                    )
                # print("RSS check:",time.time()-t0)
                if conf.simulation_config["safetyguard_flag"]:
                    RSS_excution_flag = (
                        self.RSS_model.cav_veh.RSS_control and not self.highly_critical
                    )
                else:
                ...
                if not RSS_excution_flag:
                    ...
                    if (
                        acc > conf.acc_high
                        or acc < conf.acc_low
                        or abs(acc_lat) > acc_lat_max
                    ):
                        acc_dict = {
                            "acc_x": np.clip(acc, conf.acc_low, conf.acc_high),
                            "acc_y": np.clip(acc_lat, -acc_lat_max, acc_lat_max),
                        }
                        action_list = fsolve(
                            self.helper_solve_for_control_inputs,
                            [float(action[0]), float(action[1])],
    ...
    def cav_obs_space(self):
        if (
            self._recent_cav_obs_space is None
            or self.vehicle.simulator.get_time()
            != self._recent_cav_obs_space["time_step"]
        ):
            ...
            if self.NN_metric is not None:
                self._recent_cav_obs_space["neuralmetric_obs"] = (
                    self.NN_metric.normalize(
                        self.vehicle.simulator.env.get_observation_neuralmetric()
                    )
                )
    ...
    def highly_critical(self):
        if (
            self._recent_critical is None
            or self.vehicle.simulator.get_time() != self._recent_critical["time_step"]
        ):
    ...
    def update_critical(self):
        ...
        if conf.env_mode == "NADE":
            if not conf.simulation_config["neuralmetric_flag"]:
                ...
                if conf.simulation_config["metric"] == "SMAR":
                    for (
                        global_controller
                    ) in self.vehicle.simulator.env.global_controller_instance_list:
                        if global_controller.veh_type == "BV":
                            criticality = np.sum(
                                global_controller.original_NADE_decision.bv_criticality_list
                            )
                            break
                elif conf.simulation_config["metric"] == "TTC":
                    criticality = float(self.get_ttc() < 3.0)
                elif conf.simulation_config["metric"] == "WTTC":
                    criticality = float(self.get_wttc() < 2.5)
                else:
                    criticality = 0.0
            else:
            ...
            if conf.precise_criticality_flag:
            ...
            elif conf.precise_weight_flag:
                snapshot_weight_list = (
                    self.vehicle.simulator.env.global_controller_instance_list[
                        0
                    ].control_log["weight_list_per_simulation"]
                )
                current_weight = self.vehicle.simulator.env.info_extractor.episode_log[
                    "weight_episode"
                ] * reduce(lambda x, y: x * y, snapshot_weight_list)
                if criticality > 0 and current_weight < conf.precise_weight_threshold:
                    updated_critical["highly_critical"] = True
                elif criticality > 0:
                    updated_critical["slightly_critical"] = True
            else:
                if criticality > 0:
                    ...
                    obs = self.cav_obs_space
                    _, updated_critical["slightly_critical"] = (
                        RLControllerNew.calculate_cav_criticality(obs)
                    )
    ...
    def calculate_cav_criticality(train_state, prediction_time=2.0):
        ...
        for veh_id in CAV_context.keys():
            all_obs[veh_id] = RLControllerNew.plain_process_information(
                veh_id, CAV_context
            )
        # CAV_obs = all_obs["CAV"]
        ...
        if criticality == 0:
            CAV_context_new = RLControllerNew.helper_predict_new_cav_context(
                CAV_context, all_obs, prediction_time - 1.0
            )
            all_obs_new = {}
            for veh_id in CAV_context_new.keys():
                all_obs_new[veh_id] = RLControllerNew.plain_process_information(
                    veh_id, CAV_context_new
                )
            CAV_obs_new = all_obs_new["CAV"]
            considered_bv_list_new = RLControllerNew.find_considered_bv_list(
                CAV_context_new
            )
            full_obs_new = RLControllerNew.get_full_obs_from_cav_obs_and_bv_list(
                CAV_obs_new, CAV_context_new, considered_bv_list_new
            )
            bv_critical_list_new = RLControllerNew.help_criticality_list(
                CAV_obs_new,
                CAV_context_new,
                full_obs_new,
                considered_bv_list_new,
                all_obs_new,
            )
    ...
    def plain_process_information(ego_id, cav_context):
        information = {}
        information["Ego"] = RLControllerNew.helper_plain_process_information(
            ego_id, ego_id, cav_context
        )
        ego_lane_index = information["Ego"]["lane_index"]
        leader, follower = RLControllerNew.helper_find_leader_follower(
            ego_id, cav_context, ego_lane_index, 0
        )
        leftleader, leftfollower = RLControllerNew.helper_find_leader_follower(
            ego_id, cav_context, ego_lane_index, 1
        )
        rightleader, rightfollower = RLControllerNew.helper_find_leader_follower(
            ego_id, cav_context, ego_lane_index, -1
        )
        information["Lead"] = RLControllerNew.helper_plain_process_information(
            leader, ego_id, cav_context
        )
        information["Foll"] = RLControllerNew.helper_plain_process_information(
            follower, ego_id, cav_context
        )
        information["LeftLead"] = RLControllerNew.helper_plain_process_information(
            leftleader, ego_id, cav_context
        )
        information["LeftFoll"] = RLControllerNew.helper_plain_process_information(
            leftfollower, ego_id, cav_context
        )
        information["RightLead"] = RLControllerNew.helper_plain_process_information(
            rightleader, ego_id, cav_context
        )
        information["RightFoll"] = RLControllerNew.helper_plain_process_information(
            rightfollower, ego_id, cav_context
        )
        return information
    ...
    def helper_plain_process_information(veh_id, ego_id, cav_context):
        ...
        if veh_id == ego_id:
            ...
            if cav_context[veh_id][66][0] > cav_context[ego_id][66][0]:
                distance = (
                    cav_context[veh_id][66][0]
                    - cav_context[ego_id][66][0]
                    - RLControllerNew.veh_length
                )
            else:
                distance = (
                    cav_context[ego_id][66][0]
        ...
        for i in range(len(RLControllerNew.road_geo)):
            ...
            if (
                cav_context[veh_id][66][1] >= road_bound[0]
                and cav_context[veh_id][66][1] <= road_bound[1]
            ):
                lane_index = i
                break
        ...
        veh["lateral_offset"] = (
            cav_context[veh_id][66][1] - RLControllerNew.road_center[lane_index]
        )
    ...
    @staticmethod
    # @profile
    def helper_find_leader_follower(
        ego_id, cav_context, ego_lane_index, relative_lane_index
    ):
    ...
    def get_full_obs_from_cav_obs_and_bv_list(CAV_obs, CAV_context, considered_bv_list):
        ...
        for veh_id in considered_bv_list:
            full_obs[veh_id] = RLControllerNew.helper_plain_process_information(
                veh_id, "CAV", CAV_context
            )
        return full_obs

    @staticmethod
    # @profile
    def help_criticality_list(
        CAV_obs, CAV_context, full_obs, considered_bv_list, BVs_obs
    ):
        ...
        for bv_id in considered_bv_list:
            ...
            bv_criticality, criticality_array = (
                RLControllerNew.helper_decompose_decision(
                    BV_obs,
                    CAV_obs,
    ...
    def helper_decompose_decision(
        ...
        bv_criticality, criticality_array = -np.inf, np.zeros(
        ...
        if not ((0.99999 <= bv_left_prob <= 1) or (0.99999 <= bv_right_prob <= 1)):
            bv_criticality, criticality_array, _, _ = (
                TreeSearchNADEBackgroundController._calculate_criticality(
                    BV_obs,
                    CAV,
                    SM_LC_prob,
                    full_obs,
    ...
    def helper_predict_new_cav_context(CAV_context, all_obs, prediction_time):
        ...
        for veh_id in CAV_context.keys():
            ...
            new_x = (
                current_x
                + long_speed * prediction_time
                + 0.5 * acc * (prediction_time**2)
            )
            if veh_id != "CAV":  # BV's lane change must be completed
                ...
                new_y = np.clip(
                    new_y_tmp,
                    RLControllerNew.road_center[0],
                    RLControllerNew.road_center[-1],
                )
                if new_y_tmp != new_y:
                    new_lat_speed = 0
                else:
    ...
    def get_ttc(self):
        ...
        if obs["Lead"] is not None:
            distance_front, ttc_front = self.get_ttc_helper(obs["Lead"], obs["Ego"])
        else:
            distance_front, ttc_front = 10000, 10000
        if obs["Foll"] is not None:
            distance_back, ttc_back = self.get_ttc_helper(obs["Ego"], obs["Foll"])
        else:
            distance_back, ttc_back = 10000, 10000
        return min(ttc_front, ttc_back)

    def get_ttc_helper(self, lead_obs, follow_obs):
    ...
    def get_wttc(self):
        wttc_dic = {}
        av_obs = self.vehicle.observation.information

        av_state = av_obs["Ego"]
        bv_idx = ["Lead", "LeftLead", "RightLead", "Foll", "LeftFoll", "RightFoll"]

        for bv in bv_idx:
            if av_obs[bv] != None:
                bv_state = av_obs[bv]
                bv_id = bv_state["veh_id"]
                wttc_dic[bv_id] = self.get_WTTC_helper(av_state, bv_state)

        wttc_val = list(wttc_dic.values())
        return min(wttc_val)

    def get_WTTC_helper(self, av_state: dict, bv_state: dict, a1=2, a2=1):
        """Assuming the vehicles are running horizontal, return a WTTC score between two vehicles a: AV and b: BV"""
        func_params = np.zeros(5)
        xa, ya = av_state["position"]
        vax, vay = av_state["velocity"], 0
        xb, yb = bv_state["position"]
        vbx, vby = bv_state["velocity"], 0

        func_params[0] = -1 / 4
        func_params[1] = 0
        func_params[2] = (vbx - vax) ** 2 / a1**2 + (vby - vay) ** 2 / a2**2
        func_params[3] = (
            2 * (vbx - vax) * (xb - xa) / a1**2 + 2 * (vby - vay) * (yb - ya) / a2**2
        )
        func_params[4] = (xb - xa) ** 2 / a1**2 + (yb - ya) ** 2 / a2**2

        sol = np.roots(func_params)
        wttc = np.real(np.min(sol[(np.imag(sol) == 0) & (sol > 0)])).item()
        vec = np.array([xa - xb + (vax - vbx) * wttc, ya - yb])
