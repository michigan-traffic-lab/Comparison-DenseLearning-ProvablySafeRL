...
class NADEInfoExtractor(InfoExtractor):
    def __init__(self, env):
        super().__init__(env)
        self.episode_log = {"episode_info": None, 
                            "collision_result": None, "collision_id": None, 
                            "weight_episode": 1, "current_weight": 1, "weight_step_info":{}, "weight_list_step": {},
                            "crash_decision_info": None, "decision_time_info": {}, 
                            "drl_epsilon_step_info": {}, "real_epsilon_step_info": {}, 
                            "criticality_step_info": {}, "drl_obs_step_info":{},
                            "ndd_step_info": {}, "cav_mean_speed": 0}        
        self.record = {}
        self.initial_log = {}
        self.cav_speed_list = []
        ...
        if stop:
            ...
            if 1 in self.reason: # have crash
                ...
                if not conf.nade_offline_collection:
                    ...
                    self.episode_log["NADE_info"] = self.env.global_controller_instance_list[0].nade_info
            else:
                self.episode_log["collision_result"] = 0
                if conf.more_info_critical or conf.experiment_config["log_mode"] == "offlinecollect" or conf.experiment_config["log_mode"] == "offlinecollect_all" or (self.env.simulator.input is not None and self.env.simulator.replay_flag):
                    self.episode_log["CAV_info"] = self.cav_info
                    self.episode_log["NADE_info"] = self.env.global_controller_instance_list[0].nade_info
            ...
            # if 1:
                self.episode_log["decision_time_info"] = None # will delete the most heavy part, only log the overall info
                ...
                self.episode_log["crash_decision_info"] = None
            # elif self.episode_log["weight_episode"] < 0.99:
            # elif abs(self.episode_log["weight_episode"]-1) < 1e-2 and self.cav_info[list(self.cav_info.keys())[-1]]["criticality"]>0.01:
            #     self.episode_log["decision_time_info"] = None
            #     self.episode_log["crash_decision_info"] = None
            ...
            if 7 in self.reason or 8 in reason:
                save_dir = os.path.join(self.save_dir, "speed_out_of_range")
            if 10 in self.reason:
                save_dir = os.path.join(self.save_dir, "infeasible")
            ...
            # if 1:
                ...
                if not conf.compress_log_flag:
                    ...
                    with open(save_dir + "/" + str(self.env.simulator.worker_id) + ".json", "a") as json_file:
                        json.dump(self.episode_log, json_file)
                        json_file.write('\n')
                self.weight_result = float(self.episode_log["weight_episode"])
                with open(os.path.join(self.save_dir, 'weight_' + str(self.env.simulator.worker_id) + ".json"), "a") as json_file:
                    json.dump({'Episode':self.episode_log["episode_info"]["id"], 'crash_weight': self.weight_result, 'reason': self.reason}, json_file)
                    json_file.write('\n')
                # print("CRASH WEIGHT RESULT:", self.weight_result)
            elif 10 in self.reason:
                save_dir = os.path.join(self.save_dir, "infeasible")
                os.makedirs(save_dir, exist_ok=True)
                if not conf.compress_log_flag:
                    with open(save_dir + "/"+str(self.episode_log["episode_info"]["id"]) + ".json", 'w') as json_file:
                ...
                self.log_safe_flag = False
                if conf.more_info_critical and self.cav_info != {} and "NADE_info" in self.episode_log.keys():
                    ...
                    highly_crit_list = [self.episode_log["CAV_info"][step]["NADE_critical"] for step in self.episode_log["CAV_info"]]
                    if self.episode_log["weight_episode"] < 1 or self.env.min_three_circle_distance < 4:
                        self.log_safe_flag = sum(highly_crit_list)>0
                        #self.log_safe_flag = True
                elif conf.experiment_config["log_mode"] == "offlinecollect_all":
                    self.log_safe_flag = True
                elif self.env.simulator.input is not None and self.env.simulator.replay_flag:
                    self.log_safe_flag = True
                if self.log_safe_flag:
                    save_dir = os.path.join(self.save_dir, "tested_and_safe")
                    os.makedirs(save_dir, exist_ok=True)
    ...
    def get_snapshot_info(self, control_info=None):
        ...
        self.episode_log["weight_episode"] = self.episode_log["weight_episode"] * reduce(lambda x,y:x * y,snapshot_weight_list)
        ...
        # if not isclose(self.episode_log["current_weight"], 1):
            ...
            self.episode_log["criticality_step_info"][time_step] = self.get_nadecriticality_this_step()
        ...
        all_vehicle_list = self.env.vehicle_list
        log_vehicle_candidate_ids = ["CAV"] + [vehicle.id for vehicle in self.env.global_controller_instance_list[0].nade_candidates]
        desicion_time_info_step = {}
        desicion_time_info_step["predicted_full_obs"] = self.env.global_controller_instance_list[0].predicted_full_obs
        desicion_time_info_step["predicted_traj_obs"] = self.traj_to_info(self.env.global_controller_instance_list[0].predicted_traj_obs)
        for veh_id in log_vehicle_candidate_ids:
            desicion_time_info_step[veh_id] = all_vehicle_list[veh_id].controller.ego_info
            desicion_time_info_step[veh_id]["action"] = all_vehicle_list[veh_id].controller.action
            desicion_time_info_step[veh_id]["observation"] = copy.deepcopy(all_vehicle_list[veh_id].observation.information)
            if veh_id == "CAV":
                desicion_time_info_step[veh_id]["context"] = copy.deepcopy(all_vehicle_list[veh_id].observation.context)
                desicion_time_info_step[veh_id]["SM_LC_prob"] = list(self.env.global_controller_instance_list[0].SM_LC_prob)
            if veh_id != "CAV":
                desicion_time_info_step[veh_id]["ndd_pdf"] = list(map(float, all_vehicle_list[veh_id].controller.ndd_pdf))
                desicion_time_info_step[veh_id]["critical_pdf"] = list(map(float, all_vehicle_list[veh_id].controller.bv_criticality_array))
                desicion_time_info_step[veh_id]["NADE_flag"] = all_vehicle_list[veh_id].controller.NADE_flag
                desicion_time_info_step[veh_id]["weight"] = all_vehicle_list[veh_id].controller.weight
                desicion_time_info_step[veh_id]["ndd_possi"] = all_vehicle_list[veh_id].controller.ndd_possi
                desicion_time_info_step[veh_id]["critical_possi"] = all_vehicle_list[veh_id].controller.critical_possi
                desicion_time_info_step[veh_id]["epsilon_pdf_array"] = list(all_vehicle_list[veh_id].controller.epsilon_pdf_array)
                desicion_time_info_step[veh_id]["bv_challenge_array"] = list(all_vehicle_list[veh_id].controller.bv_challenge_array)
        self.episode_log["decision_time_info"][time_step] = desicion_time_info_step
        
        if not conf.more_info_critical and conf.experiment_config["log_mode"] in ["offlinecollect","crash","offlinecollect_all"]:
            self.cav_info[time_step] = {
                "RSS_control_flag": cav.controller.RSS_model.cav_veh.RSS_control if hasattr(cav.controller, 'RSS_model') else None,
                "RSS_control": cav.controller.RSS_model.cav_veh.action if hasattr(cav.controller, 'RSS_model') else None,
                "CAV_action": cav.controller.action if hasattr(cav.controller, 'action') else None,
                "NADE_critical": cav.controller.highly_critical if hasattr(cav.controller, 'highly_critical') else False,
                "slightly_critical": cav.controller.slightly_critical if hasattr(cav.controller, 'slightly_critical') else False,
                "criticality": cav.controller._recent_critical["criticality"] if hasattr(cav.controller, '_recent_critical') else 0.0,
            }
        ...
        if hasattr(cav.controller, 'RSS_model') and cav.controller.RSS_model.cav_veh.RSS_control:
            self.RSS_step_num  += 1
    ...
    def handle_fcd_files(self):
        ...
        if 1 in self.reason:
            ...
            if conf.compress_log_flag:
                if os.path.isfile(file):
                    with open(file) as xml_file:
                        ...
                        with open(save_dir + "/" + str(self.env.simulator.worker_id) + ".fcd.json", "a") as json_file:
                            json.dump(data_dict, json_file)
                            json_file.write('\n')
                    os.remove(file)
        elif 10 in self.reason:
            save_dir = os.path.join(self.env.simulator.experiment_path, "infeasible")
            if conf.compress_log_flag:
                if os.path.isfile(file):
                    with open(file) as xml_file:
                        data_dict = xmltodict.parse(xml_file.read())
            ...
            save_dir = os.path.join(self.env.simulator.experiment_path, "leave_network")
            if conf.compress_log_flag:
                if os.path.isfile(file):
                    with open(file) as xml_file:
                        data_dict = xmltodict.parse(xml_file.read())
            ...
            save_dir = os.path.join(self.env.simulator.experiment_path, "tested_and_safe")
            if conf.compress_log_flag:
                if os.path.isfile(file):
                    with open(file) as xml_file:
                        data_dict = xmltodict.parse(xml_file.read())
                ...
                if os.path.isfile(file):
                    shutil.move(file, os.path.join(save_dir,self.env.simulator.output_filename+"."+"fcd.xml")) 
        else:
            if os.path.isfile(file):
                os.remove(file)