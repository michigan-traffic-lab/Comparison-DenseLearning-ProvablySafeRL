...
from gym_commonroad_sumo.NDE_RL_NADE.utils_sat import separating_axis_theorem
...
def is_lane_change(obs_ego):
    ...
    if obs_ego["veh_id"] != "CAV":
        ...
        if abs(obs_ego["lateral_speed"]) > 4:
            return True
        elif obs_ego["prev_action"] is not None and "lateral" in obs_ego["prev_action"] and obs_ego["prev_action"]["lateral"] != "central":
            return True
        else:
            return False
...
def _Mobil_surraget_model(cav_obs, lane_index):
    ...
    new_following_pred_a = acceleration(ego_vehicle=new_following, front_vehicle=cav_info)
...
def _get_Surrogate_CAV_action_probability(cav_obs):
    ...
    for lane_index in lane_index_list:
        LC_safety_flag, gain = _Mobil_surraget_model(cav_obs=cav_obs, lane_index=lane_index)
    ...
    obtained_LC_prob_for_sharing = np.clip(remap(total_gain, [0, conf.SM_MOBIL_max_gain_threshold], [0, max_remaining_LC_prob]), 0, max_remaining_LC_prob)
...
def detect_crash_three_circle(cav_center_list, veh_center_list):
    ...
    for cav_center in cav_center_list:
        for veh_center in veh_center_list:
            distan_power = (veh_center[0]-cav_center[0])**2+(veh_center[1]-cav_center[1])**2
            distan_power_list.append(distan_power)
    
    min_three_circle_distance = math.sqrt(min(distan_power_list))
    if min_three_circle_distance <= 2*circle_r:
        return True, min_three_circle_distance
    else:
        return False, min_three_circle_distance
