...
class NDEInfoExtractor(InfoExtractor):
    ...
    def get_terminate_info(self, stop, reason, additional_info):
        ...
        if stop:
            ...
            if 1 not in reason and self.env.simulator.experiment_path is not None and self.env.simulator.output is not None and not (conf.experiment_config["log_mode"] == "all"):
