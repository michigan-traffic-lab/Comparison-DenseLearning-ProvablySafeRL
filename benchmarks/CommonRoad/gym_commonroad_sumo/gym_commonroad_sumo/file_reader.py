...
SUPPORTED_COMMONROAD_VERSIONS = {'2018b', '2020a', '2017a'}
...
class ScenarioFactory:
    ...
    def create_from_xml_node(cls, xml_node: ElementTree.Element, dt: float, benchmark_id: str, commonroad_version: str,
        ...
        if commonroad_version == '2017a':
            large_num = 10000
            scenario.add_objects(cls._obstacles_2017a(xml_node, scenario.lanelet_network, lanelet_assignment))
    ...
    @classmethod
    def _obstacles_2017a(cls, xml_node: ElementTree.Element, lanelet_network: LaneletNetwork,
...
class DynamicObstacleFactory(ObstacleFactory):
    ...
    def create_from_xml_node(cls, xml_node: ElementTree.Element, lanelet_network: LaneletNetwork,
        ...
        if xml_node.find('trajectory') is not None:
            ...
            initial_state = trajectory.state_at_time_step(0)
            if lanelet_assignment is True:
                rotated_shape = shape.rotate_translate_local(initial_state.position, initial_state.orientation)
                initial_shape_lanelet_ids = set(lanelet_network.find_lanelet_by_shape(rotated_shape))
                initial_center_lanelet_ids = set(lanelet_network.find_lanelet_by_position([initial_state.position])[0])
...
class GoalRegionFactory:
    ...
    def create_from_xml_node(cls, xml_node: ElementTree.Element, lanelet_network: LaneletNetwork)\
        ...
        for idx, goal_node in enumerate(xml_node.findall('goalRegion')):
            goal_state_node = goal_node.find('state') 
            state_list.append(StateFactory.create_from_xml_node(goal_state_node, lanelet_network, is_goal_region=True))
...
class StateFactory:
    @classmethod
    def create_from_xml_node(cls, xml_node: ElementTree.Element, lanelet_network: Union[LaneletNetwork, None] = None, is_goal_region = False)\
            -> State:
        state_args = dict()
        if xml_node.find('position') is not None:
        ...
        if xml_node.find('yawRate') is not None and not is_goal_region:
            yaw_rate = read_value_exact_or_interval(xml_node.find('yawRate'))
            state_args['yaw_rate'] = yaw_rate
        if xml_node.find('slipAngle') is not None and not is_goal_region:
