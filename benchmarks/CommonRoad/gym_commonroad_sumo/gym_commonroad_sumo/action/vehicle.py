...
from gym_commonroad_sumo.utils.scenario import make_valid_orientation
...
class ContinuousVehicle(Vehicle):
    ...
    def propagate_one_time_step(self, current_state: State, action: np.ndarray, action_base: str) -> State:
        ...
        if self.vehicle_model == VehicleModel.PM:
            ...
            return self.vehicle_dynamic.array_to_state(x_current, time_step=current_state.time_step + 1)
        elif self.vehicle_model == VehicleModel.PMNonlinear:
            kwarg = {
                "position": np.array([x_current[0], x_current[1]]),
                "velocity": x_current[2],
...
class QPDynamics(VehicleDynamics):
    ...
    def forward_simulation(self, x: np.array, u: np.array, dt: float, throw: bool = True) -> np.array:
        """
        ...
        """
        # longitudinal x: (s, v, a, j)
        A_long = np.array(
            [[1, dt, (dt ** 2.) / 2., (dt ** 3.) / 6.],
             [0, 1., dt, (dt ** 2.) / 2.],
             [0, 0, 1., dt],
             [0, 0, 0, 1]])  # 4x4
        B_long = np.array([[(dt ** 4.) / 24.], [(dt ** 3.) / 6.], [(dt ** 2.) / 2.], [dt]])  # 4x1
        x1_long = np.dot(A_long, x[0, :]) + np.squeeze(np.dot(B_long, u[0]))  # j_dot

        j_max = 2*self.parameters.longitudinal.a_max / dt

        if x1_long[2] > self.parameters.longitudinal.a_max:
            x1_long[2] = self.parameters.longitudinal.a_max
            if x1_long[3] > 0:
                x1_long[3] = 0
        if x1_long[2] < -self.parameters.longitudinal.a_max:
            x1_long[2] = -self.parameters.longitudinal.a_max
        ...
        # lateral x: (d, θ, κ, ˙κ)
        v = x1_long[1]
        theta = 0.0  # x_ref.reference[k].theta
        ...
        D = np.array([-dt * v, 0, 0, 0])

        x1_lat = np.dot(A_lat, x[1, :]) + np.squeeze(np.dot(B_lat, u[1])) + np.dot(theta, D)  # kappa_dot_dot
        ...
        if x1_lat[3] > kappa_dot_max:
            x1_lat[3] = kappa_dot_max
        if x1_lat[3] < -kappa_dot_max:
            x1_lat[3] = -kappa_dot_max

        return np.array([x1_long, x1_lat])
    ...
    def _state_to_array(self, state: State, steering_angle_default=0.0) -> Tuple[np.array, int]:
        ...
        values = np.array([
            [state.position[0] - self.parameters.b * np.cos(state.orientation), state.velocity, state.acceleration,
             state.jerk],
