...
from gym_commonroad_sumo.file_reader import LaneletNetworkFactory, DynamicObstacleFactory, StaticObstacleFactory
...
class ScenarioWrapper(AbstractScenarioWrapper):
    def __init__(self, sumo_cfg_file=None):
    ...
    def load_cr_map(self, cr_map_file):
        # Load the commonroad map file

        tree = et.parse(cr_map_file)
        Lanelet_net = LaneletNetworkFactory.create_from_xml_node(tree)
        dynamic_obstacles = dict()
        static_obstacles = dict()
        for o in tree.findall('obstacle'):
            if o.find('role').text == 'static':
                static_obstacles[int(o.attrib['id'])] = StaticObstacleFactory.create_from_xml_node(o, Lanelet_net, False)
            elif o.find('role').text == 'dynamic':
                dynamic_obstacles[int(o.attrib['id'])] = DynamicObstacleFactory.create_from_xml_node(o, Lanelet_net, False)
        return Lanelet_net, dynamic_obstacles, static_obstacles

    def initialize_sumo(self, scenario: Scenario, cr_map_file, time_step_size):
        # initialize the sumo scenario wrapper from a commonroad scenario

        lanelet_net, dynamic_obstacles, static_obstacles = self.load_cr_map(cr_map_file)
        scenario_new = Scenario(dt=time_step_size)
        scenario_new.lanelet_network = lanelet_net
        scenario_new.scenario_id = scenario.scenario_id
        scenario_new._dynamic_obstacles = dynamic_obstacles
        scenario_new._static_obstacles = static_obstacles
        self.initial_scenario = scenario_new
