...
from gym_commonroad_sumo.simulation.util import *
...
from gym_commonroad_sumo.simulation.prefix import EGO_ID_START, ID_DICT
...
class SumoSimulation:
    ...
    def initialize(self, conf: DefaultConfig,
        ...
        output_file_path = os.path.join(output_file,'data.xml')
        ...
        # load sumo initialization
        if sumo_initialization is not None:
            for veh in sumo_initialization:
                traci.vehicle.add(veh.get('id'),veh.find('route').text)
                traci.vehicle.moveTo(veh.get('id'),veh.find('lane').text,float(veh.find('pos').text))
                traci.vehicle.setPreviousSpeed(veh.get('id'),float(veh.find('speed').text))
                traci.vehicle.setAcceleration(veh.get('id'),float(veh.find('acc').text),0.1)
                traci.vehicle.setType(veh.get('id'),veh.find('type').text)
    ...
    def init_ego_vehicles_from_planning_problem(
        ...
        if planning_problem_set is not None:
            ...
            for planning_problem in planning_problem_set.planning_problem_dict.values():
                ...
                if sumo_id in self.ids_sumo2cr[EGO_ID_START].keys():
                    sumo_id = EGO_ID_START + str(new_id)
                cr_id = self._create_cr_id(type=EGO_ID_START, sumo_id=sumo_id, sumo_prefix=EGO_ID_START)
    ...
    def commonroad_scenario_at_time_step(self, time_step: int, add_ego=False, start_0=True) -> Scenario:
        ...
        """
        cr_scenario = self.scenarios.create_minimal_scenario()

        cr_scenario.add_objects(self._get_cr_obstacles_at_time(time_step, add_ego=add_ego, start_0=start_0))
        return cr_scenario
    ...
    def commonroad_scenarios_all_time_steps(self) -> Scenario:
        ...
        """
        cr_scenario = self.scenarios.create_full_meta_scenario()

        cr_scenario.add_objects(self._get_cr_obstacles_all())
        return cr_scenario
    ...
    def _subscribe_new(self, new_ids: List[str], domain=traci.vehicle):
        ...
        #    print("subscribe_vehicles:",vehicle_ids)
    ...
    def _fetch_sumo_vehicles(self, time_step: int, new_ids: List[str]):
        ...
        for veh_id, vehicle_state in vehicle_states.items():
            ...
            if veh_id in new_ids:
                ...
                self.obstacle_states[time_step][self.ids_sumo2cr['obstacleVehicle'][veh_id]] = state
            elif not self._silent and veh_id not in self.ids_sumo2cr[EGO_ID_START]:
            ...
            if not self._silent and self.dummy_ego_simulation and veh_id in self.ids_sumo2cr[EGO_ID_START]:
                ego_veh = self.ego_vehicles[self.ids_sumo2cr[EGO_ID_START][veh_id]]
    ...
    def _get_cr_obstacles_at_time(self, time_step: int,
        ...
        """
        vehicle_dict: Dict[int, State] = copy.deepcopy(self.obstacle_states[time_step])
    ...
    def _get_cr_obstacles_all(self) -> List[DynamicObstacle]:
        ...
        obstacle_states = copy.deepcopy(self.obstacle_states)
        veh_state = {}
        for time_step, veh_dicts in obstacle_states.items():
        ...
        for veh_id, time_dict in veh_state.items():
            ...
            if len(state_list) > 1:
                ...
                self.logger.debug(
                    f'Vehicle {veh_id} has been simulated less than 1 time steps. Not converted to cr obstacle.'
                )
        return obstacles
