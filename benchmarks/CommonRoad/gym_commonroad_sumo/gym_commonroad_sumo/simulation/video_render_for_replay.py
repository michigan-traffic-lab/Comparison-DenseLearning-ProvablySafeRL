...
from commonroad.scenario.obstacle import ObstacleType, DynamicObstacle, StaticObstacle, ObstacleRole, SignalState, PhantomObstacle, \
...
class MPRenderer(IRenderer):
    ...
    def __init__(self, draw_params: Union[ParamServer, dict, None] = None,
                 ...
                 figsize: Union[None, Tuple[float, float]] = None, focus_obstacle: Union[None, Obstacle] = None, action_dict=None, action_dict2=None):
        """
        ...
        if action_dict:
            ...
            self.action_dict = []
        if action_dict2:
            self.action_dict2 = action_dict2
        else:
            self.action_dict2 = []
        self.plotted_step = [0] * len(self.action_dict)
        self.plotted_step2 = [0] * len(self.action_dict2)
    ...
    def remove_dynamic(self) -> None:
        ...
        for t in self.dynamic_labels.values():
            t.set_visible(False)
        for t in self.dynamic_labels2.values():
            t.set_visible(False)
        self.dynamic_labels.clear()
        self.dynamic_labels2.clear()
    ...
    def render_dynamic(self) -> List[artists.Artist]:
        ...
        for s in self.dynamic_labels.keys():
            t = self.dynamic_labels[s]
            self.ax.add_artist(t)
        for s in self.dynamic_labels2.keys():
            t = self.dynamic_labels2[s]
            self.ax.add_artist(t)
    ...
    def draw_dynamic_obstacle(self, obj: DynamicObstacle, draw_params: Union[ParamServer, dict, None],
        ...
        if state is not None and obj.state_at_time(1) is not None and obj.state_at_time(1).time_step == 1:
            tmp_step = state.time_step
            if tmp_step < len(self.action_dict) and self.plotted_step[tmp_step] == 0 and len(self.action_dict[tmp_step]) == 3:
                self.plotted_step[tmp_step] = 1
                xlim = self.ax.get_xlim()
                ylim = self.ax.get_ylim()
                center_x = (xlim[0] + xlim[1]) / 2
                center_y = (ylim[0] + ylim[1]) / 2
                action = self.action_dict[tmp_step][0]
                info = self.action_dict[tmp_step][1]
                ego_state = self.action_dict[tmp_step][2]
                self.dynamic_labels[tmp_step] = text.Text(center_x - 105, center_y + 20, 
                                                    f" Info: \n {info} \n Ego information at time step {tmp_step} \n State: position=[{ego_state[0][0]:.3f},{ego_state[0][1]:.3f}] velocity={ego_state[1]:.4f} orientation= {ego_state[2]:.4f} \n Real control input: acceleration={action[0]:.3f}, yaw rate={action[1]:.5f}", 
                                                    clip_on=True, fontsize=8, zorder=ZOrders.LABELS)
                
            if tmp_step < len(self.action_dict2) and self.plotted_step2[tmp_step] == 0 and len(self.action_dict2[tmp_step]) == 3:
                self.plotted_step2[tmp_step] = 1
                xlim = self.ax.get_xlim()
                ylim = self.ax.get_ylim()
                center_x = (xlim[0] + xlim[1]) / 2
                center_y = (ylim[0] + ylim[1]) / 2
                action = self.action_dict2[tmp_step][0]
                info = self.action_dict2[tmp_step][1]
                ego_state = self.action_dict2[tmp_step][2]
                self.dynamic_labels2[tmp_step] = text.Text(center_x - 105, center_y - 50, 
                                                    f" Comparison info: \n {info} \n Ego information at time step {tmp_step} \n State: position=[{ego_state[0][0]:.3f},{ego_state[0][1]:.3f}] velocity={ego_state[1]:.4f} orientation= {ego_state[2]:.4f} \n Real control input: acceleration={action[0]:.3f}, yaw rate={action[1]:.5f}", 
                                                    clip_on=True, fontsize=8, zorder=ZOrders.LABELS)
    ...
    def draw_lanelet_network(self, obj: LaneletNetwork, draw_params: Union[ParamServer, dict, None],
        ...
        show_label = True #draw_params.by_callstack(call_stack, ('lanelet', 'show_label'))
        ...
        for i_lanelet, lanelet in enumerate(lanelets):
            ...
            # Draw labels
            if 0: #if show_label or show_intersection_labels or draw_traffic_signs:
                strings = []
                if show_label:
        ...
        self.static_collections.append(
                collections.PolyCollection(vertices_fill, transOffset=None, zorder=ZOrders.LANELET_POLY,
...
def create_video_for_replay(scenario: Scenario,
                 output_folder: str,
    ...
    frame_count_padding = 0 #int(1 / scenario.dt)
    ...
    frame_count = max([len(prediction.trajectory.state_list) for prediction in trajectory_pred], default=0) + frame_count_padding
    ...
    dpi = 150
    figsize = (5, 5)
    rnd = MPRenderer(figsize=figsize, draw_params={"axis_visible": False}, action_dict=action_dict, action_dict2=action_dict2)
    rnd.ax.axes.get_xaxis().set_visible(False)
    (ln,) = plt.plot([], [], animated=True)
    ego_color = ['green','yellow','red']
    ...
    def init_plot():
        ...
        ego_draw_params = []
        for ego_id in range(len(dynamic_obstacles_ego)):
            ego_draw_params.append({'time_begin': 0, 'time_end': 0, "axis_visible": False,
                                            'dynamic_obstacle': {'vehicle_shape': {"occupancy": {"shape": {"rectangle": {
                                                "facecolor": ego_color[ego_id]}}}}}})
        if dynamic_obstacles_ego is not None:
            rnd.draw_list(dynamic_obstacles_ego, draw_params=ego_draw_params)
    ...
    def animate_plot(frame):
        ...
        ego_draw_params = []
        for ego_id in range(len(dynamic_obstacles_ego)):
            ego_draw_params.append({'time_begin': frame, 'time_end': frame,
                                            'dynamic_obstacle': {'vehicle_shape': {"occupancy": {"shape": {"rectangle": {
                                                "facecolor": ego_color[ego_id]}}}}}})
        if dynamic_obstacles_ego is not None:
            rnd.draw_list(dynamic_obstacles_ego,
                          draw_params=ego_draw_params)
    ...
    try:
        file_name = filename + os.extsep + file_type
        anim.save(os.path.join(output_folder, file_name), dpi=dpi, writer="ffmpeg")
    except:
        file_name = filename + os.extsep + 'gif'
        anim.save(os.path.join(output_folder, file_name), dpi=dpi, writer="ffmpeg")
    plt.close(rnd.f)
    return file_name
