...
elseif isa(obstacle, 'world.Vehicle')
    ...
    if isa(map, 'world.Map')
        ...
        if ~isempty(obstacle.inLane)
            % return the current lane and all adjacent lanes with the same driving direction
            reachableLanes = prediction.Occupancy.findAllReachableLanes(map.lanes, obstacle.constraint5, obstacle.inLane);
            lane_orientation = geometry.calcAngleOfVerticesAtPosition(obstacle.position, obstacle.inLane.center.vertices);
        ...
        % check lane change
        if obstacle.constraint5
            occupancy_new = occupancy(1,:);
            if abs(obstacle.orientation-lane_orientation) > 0.01 
                for i = 1:size(occupancy,2)
                    for j = 2:size(occupancy,1)
                        if ~isempty(occupancy(j,i).vertices) & isempty(occupancy_new(i).vertices)
                            occupancy_new(i).vertices = occupancy(j,i).vertices;
                        elseif ~isempty(occupancy(j,i).vertices)
                            [verticesOcc_x, verticesOcc_y] = polybool('union', occupancy_new(i).vertices(1,:), occupancy_new(i).vertices(2,:), occupancy(j,i).vertices(1,:), occupancy(j,i).vertices(2,:));
                            occupancy_new(i).vertices = [verticesOcc_x; verticesOcc_y];
                        end
                    end
                end
            end
        else
            occupancy_new = occupancy;
        end
